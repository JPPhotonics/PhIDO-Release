...
def heater_doped_si_cband(
    ...
    doping = c << gf.path.straight(length=length - 0.4).dmovex(5.2).extrude(doping_xs)
    ...
    viac = c << gf.path.straight(length=length - 1).dmovex(5.5).extrude(viac_xs)
    ...
    m1 = c << gf.path.straight(length=length + 0.4).dmovex(4.8).extrude(m1_xs)
    ...
    via1 = c << gf.path.straight(length=71).dmovex((length - 71) / 2 + 5).extrude(
        via1_xs
    )
    ...
    m2 = c << gf.path.straight(length=75).dmovex((length - 75) / 2 + 5).extrude(m2_xs)
    ...
    pad = c << gf.path.straight(length=71).dmovex((length - 71) / 2 + 5).extrude(pad_xs)
