...
def pndiode(
    ...
    wg_prop = c << gf.path.straight(length=length).extrude(wg_prop_xs)
    ...
    wg = c << gf.path.straight(length=length - 1).dmovex(0.5).extrude(wg_xs)

    wg1_taper1 = c << gf.components.taper(
        length=20, width1=0.7, width2=0.5, port=None, layer=(1, 0)
    )
    wg1_taper1.drotate(180)
    ...
    slab = c << gf.path.straight(length=length).extrude(slab_xs)
    ...
    doping1 = c << gf.path.straight(length=length + 1.5).dmovex(-1).extrude(doping1_xs)
    ...
    doping2 = c << gf.path.straight(length=length + 1.5).dmovex(-1).extrude(doping2_xs)
    ...
    viac = c << gf.path.straight(length=length - 4).dmovex(2).extrude(viac_xs)
    ...
    m1 = c << gf.path.straight(length=length + 3).dmovex(-1.5).extrude(m1_xs)
