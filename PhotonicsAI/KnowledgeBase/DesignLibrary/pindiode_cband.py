...
def pindiode_cband(
    ...
    _doping = c << gf.path.straight(length=length - 2).dmovex(6).extrude(doping_xs)
    ...
    _viac = c << gf.path.straight(length=length - 3).dmovex(6.5).extrude(viac_xs)
    ...
    _m1 = c << gf.path.straight(length=length).dmovex(5).extrude(m1_xs)
