...
def write_sparameters_lumerical(
    ...
    **settings,
    ...
    if count == 0:
        ...
        component_extended_beyond_pml.name = f"top${count}"
    ...
    for layer, thickness in layer_to_thickness.items():
        ...
        # print(component_extended_beyond_pml.name_counters[])
        if count == 0:
            s.gdsimport(str(gdspath), "top", f"{layer[0]}:{layer[1]}")
        else:
            s.gdsimport(str(gdspath), f"top${count}", f"{layer[0]}:{layer[1]}")
    ...
    if run:
        ...
        logger.info(f"Completed simulation, compute time (minutes): {(end-start)/60}")
        ...
        if delete_fsp_files and fspdir.exists():
            ...
            # )
        sp["filepath_npz"] = str(filepath_npz)
        return sp
    os.remove(gdspath)
    # filepath_sim_settings.write_text(yaml.dump(sim_settings))
    # s['filepath_npz'] = str(filepath_npz)
    return s, sp
