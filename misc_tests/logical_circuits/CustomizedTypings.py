...
class Schematic(BaseModel):
    ...
    def plot_netlist(
        ...
        """Plots a netlist graph with networkx using spring layout for automatic node placement.
        ...
        # Adding instances as nodes and connections as edges
        for instance_name, _instance_details in netlist.instances.items():
            G.add_node(instance_name)  # Add nodes from instances

            # Each instance can contain multiple connections, handle them accordingly
            for _route_name, bundle in netlist.routes.items():
                for link_start, link_end in bundle.links.items():
                    start_instance, _ = link_start.split(",")
                    end_instance, _ = link_end.split(",")
                    if start_instance == instance_name or end_instance == instance_name:
                        G.add_edge(start_instance, end_instance)

        # Automatically determine node positions using the spring layout algorithm.
        pos = nx.spring_layout(
            G
        )  # Node positions are determined by the spring layout algorithm.

        # Generating labels for nodes based on the instance names.
        labels = {node: node for node in G.nodes()} if with_labels else None
        ...
        # plt.show()  # This line ensures that the plot is displayed when the function is called.
